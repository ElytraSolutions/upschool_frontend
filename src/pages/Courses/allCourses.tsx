...
function allCourses() {
  ...
  const getCoursesByCategory = (categoryId: number) => {
    return courses.filter(course => course.course_category_id === categoryId);
  }
    ...
    <>
      <div>
          ...
          <div className="flex flex-wrap mt-4 justify-center">
            {
              categories.map((category) => (
                <div className="py-2" key={category.id}>
                  <a href={`#${getCategoryId(category)}`} className="px-4 border-r-2">
                    {category.name}
                  </a>
                </div>
              ))
        ...
        <div id="10-weeks" className="my-10 md:max-w-6xl mx-auto ">
          {categories.map((category) => (
            <div className="p-4" key={category.id} id={getCategoryId(category)}>
              <h1 className="text-2xl font-bold text-[#242254]">
                {category.name}
              </h1>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 py-4 gap-4">
                {getCoursesByCategory(category.id).map((data) => (
                  <div className="py-4 px-0" key={data.id}>
                    <CourseCard key={data.name} data={data} />
                  </div>
                ))}
              </div>
            </div>
